...
This script compute the weighted IVR, as in (Picheny et al, 2010).
Basically, this criterion is used to recover level sets, and the weighting is
done according to the probability of the field being in a tube around the level
set.

Compared to the bare IVR, this requires computation of a gaussian CDF.
...
output_path = "/home/cedric/PHD/Dev/VolcapySIAM/volcapy/update/weighted_IVR_results_coast"
...
def main():
    ...
    # Compute the weights according to the current information.
    current_variance = updatable_cov.extract_variance(n_chunks=20)
    from volcapy.gaussian_cdf import gaussian_cdf
    lower = THRESHOLD - EPSILON
    upper = THRESHOLD + EPSILON
    weights = gaussian_cdf(updatable_mean.m, current_variance, lower, upper)
