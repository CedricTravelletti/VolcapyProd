...
def plot_excu_profile_with_data_revised(volcano_coords, data_coords_niklas, 
        coast_coords,
        ground_truth, threshold, out_file_path):
    """ Plot the x, y and z profile of the excursion set, together with Niklas
    data.

    Version for revised paper.
    ...
    f_ax1 = plt.gca()
    im = f_ax1.scatter(volcano_X[true_excursion_inds],
            volcano_Y[true_excursion_inds], c=true_excu_densities,
            cmap=cmap,
            s=8,
            marker="h",
            alpha=0.2, edgecolors='none')
    f_ax1.scatter(X_niklas_centred, Y_niklas_centred, c="black", linewidth=0.2, s=1.5,
            alpha=1.0)
    # Add the coast
    X_coast_centred = 111*1e3 * (1e-5 * (coast_coords[:, 0] - np.min(volcano_coords[:, 0])))
    Y_coast_centred = 111*1e3 * (1e-5 * (coast_coords[:, 1] - np.min(volcano_coords[:, 1])))
    f_ax1.scatter(X_coast_centred, Y_coast_centred, c="blue", linewidth=0.2, s=0.9,
            alpha=1.0)
    ...
    plt.savefig(out_file_path + "_1", bbox_inches="tight", pad_inches=0.1, dpi=400)
    plt.close()

    fig = plt.figure(figsize=(3, 5), dpi=400)
    f_ax2 = plt.gca()
    f_ax2.scatter(volcano_X[true_excursion_inds],
            volcano_Z[true_excursion_inds], c=true_excu_densities,
            cmap=cmap,
            ...
            marker="h",
            alpha=0.2, edgecolors='none')
    f_ax2.scatter(X_niklas_centred, Z_niklas, c="black", linewidth=0.2, s=1, alpha=1.0)
    f_ax2.set_aspect(1.5)
    f_ax2.tick_params(axis='both', which='major', pad=0.1)

    plt.savefig(out_file_path + "_2", bbox_inches="tight", pad_inches=0.1, dpi=400)
    plt.close()
    ...
    f_ax3.scatter(volcano_Y[true_excursion_inds],
            ...
            marker="h",
            alpha=0.2, edgecolors='none')
    f_ax3.scatter(Y_niklas_centred, Z_niklas, c="black", linewidth=0.2, s=1, alpha=1.0)
    f_ax3.set_aspect(1.5)
    f_ax3.tick_params(axis='both', which='major', pad=0.1)

    plt.savefig(out_file_path + "_3", bbox_inches="tight", pad_inches=0.1, dpi=400)
    plt.close()

def plot_excu_profile_with_visited_inds_revised(volcano_coords, data_coords_niklas, 
        visited_coords, coast_coords,
        ground_truth, threshold, out_file_path):
    """ Plot the x, y and z profile of the excursion set and the points visited
    by the strategy. Also plot Niklas data.
    ...
    # Add the coast
    f_ax1.scatter(X_coast_centred, Y_coast_centred, c="blue", linewidth=0.2, s=1,
            alpha=1.0)
    ...
    fig = plt.figure(figsize=(3, 5), dpi=400)
    f_ax2 = plt.gca()
    im = f_ax2.scatter(volcano_X[true_excursion_inds],
            volcano_Z[true_excursion_inds], c=true_excu_densities,
            cmap=cmap,
            s=4,
            marker="h",
    ...
    fig = plt.figure(figsize=(3, 5), dpi=400)
    f_ax3 = plt.gca()
    im = f_ax3.scatter(volcano_Y[true_excursion_inds],
            volcano_Z[true_excursion_inds], c=true_excu_densities,
            cmap=cmap,
            s=4,
            marker="h",
