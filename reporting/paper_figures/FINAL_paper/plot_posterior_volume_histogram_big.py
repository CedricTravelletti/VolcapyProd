...
def main():
    ...
    for sample_nr in range(1, 6):
        ...
        true_excursion_inds = (ground_truth >= threshold_big).nonzero()[:, 0]
        ...
        for i in range(11, 500):
            sample_path = os.path.join(base_results_folder,
                    "wIVR_final_big/sample_{}/post_samples/post_sample_{}.npy".format(sample_nr, i))
            sample = torch.from_numpy(np.load(sample_path))
            excursion_inds = (sample >= threshold_big).nonzero()[:, 0]
    ...
    plt.savefig("histogram_posterior_big", bbox_inches="tight", pad_inches=0.1, dpi=600)
    plt.show()
