...
ground_truth = np.load("./results/ground_truth.npy")
...
np.save(
        os.path.join(output_folder, "m_prior.npy"),
            m_prior.float().detach().numpy())
np.save(
        os.path.join(output_folder, "sigma_prior.npy"),
            sigma_prior.float().detach().numpy())
