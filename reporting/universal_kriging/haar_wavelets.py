""" Test universal kriging with Haar wavelets.
...
def main():
    ...
    # Build trends: Haar wavelets.
    x0 = volcano_coords[:, 0].mean() # Volcano center.
    y0 = volcano_coords[:, 1].mean()
    z0 = volcano_coords[:, 2].mean()

    coeff_F_haar = build_tensor_haar_basis()
    coeff_F = torch.hstack([
        torch.ones(volcano_coords.shape[0], 1),
        coeff_F_haar])

    coeff_mean = torch.vstack([
            torch.tensor([m0]),
            torch.zeros((coeff_F_haar.shape[1], 1))
            ]).reshape(-1, 1)
    coeff_cov = torch.diag(
            torch.cat([torch.tensor([200]),
            var_haar * torch.ones(coeff_F_haar.shape[0])
            ]))
